...
string Red(string str,string wd)
 {
   ...
   {
   return System.Text.RegularExpressions.Regex.Replace(str,wd, "<span style='color:#ff0000'>"+wd+"</span>", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
   }
 }
</script>
<ul class="article">
...
<a href="<%=Detail_Url(dr)%>" target="_blank" title="<%=Server.HtmlEncode(dr["title"].ToString())%>" style="<%=dr["pa_style"]%>"><%=Red(SubStr(dr["title"].ToString(),100,true),Request.QueryString["title"])%></a></span>

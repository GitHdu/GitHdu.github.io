...
%>
<div class="rw_pic"><a href="<%=Detail_Url(dr)%>"><img src="<%=dr["titlepic"].ToString()%>"></a></div>
<div class="rw_com"><a href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a><span><%=SubStr(dr["pa_introduct"].ToString(),300,true)%></span></div>
<%
 }
%>
</div><%conn.Close();%>
